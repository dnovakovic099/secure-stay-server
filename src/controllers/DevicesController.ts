...
export class DevicesController {
  ...
  async getSifelyLocks(
    ...
    try {
      ...
      if (sifelyLocks.length == 0)
        return response.status(200).json(dataNotExist());
  ...
  async getSifelyLockInfo(
    ...
    try {
      const deviceService = new DeviceService();
      const lockInfo = await deviceService.getSifelyLockInfo(
        Number(request.params.lockId)
      );
  ...
  async saveLockListingInfo(
    ...
    try {
      ...
      return response
        .status(201)
        .json(dataSaved("Device listing info saved successfully"));
    } catch (error) {
      return next(error);
    }
  }
  ...
  async getDeviceListing(
    ...
    try {
      const deviceService = new DeviceService();
      const deviceListing = await deviceService.getDeviceListing(
        request.params.lockId
      );

      if (deviceListing == null) {
        return response
          .status(200)
          .json(dataNotFound("Lock is not associated with any listing"));
      }
  ...
  async getPassCodesOfSifelyDevice(
    request: Request,
    response: Response,
    next: NextFunction
  ) {
    try {
      ...
      const passCodes = await deviceService.getCodesForSifelyDevice(
        accessToken,
        Number(lockId)
      );
  ...
  async createPassCode(
    ...
    try {
      ...
      const startDateTimestamp =
        timingOption !== 2 ? new Date(startDate).valueOf() : startDate;
      const endDateTimestamp =
        timingOption !== 2 ? new Date(endDate).valueOf() : endDate;

      await deviceService.createCodesForSifelyDevice(
        accessToken,
        lockId,
        codeName,
        codeValue,
      ...
      return response
        .status(201)
        .json(dataSaved("Passcode created successfully!!!"));
    } catch (error) {
      return next(error);
    }
  }
  ...
  async deletePassCode(
    ...
    try {
      ...
      return response
        .status(200)
        .json(dataUpdated("Passcode deleted successfully!!!"));
    } catch (error) {
      return next(error);
    }
  }
