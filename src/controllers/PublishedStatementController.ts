...
export class PublishedStatementController {
    async savePublishedStatementFromHA(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            return response.status(200).json({ message: "Published statements saved successfully", data: result });
        } catch (error) {
            next(error);
        }
    }

    async getPublishedStatements(request: Request, response: Response, next: NextFunction) {
        try {
            const publishedStatementService = new PublishedStatementService();
