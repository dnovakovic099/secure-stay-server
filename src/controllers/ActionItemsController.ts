...
export class ActionItemsController {
  ...
  async createActionItem(
    ...
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const createdActionItem = await actionItemsService.createActionItem(
        actionItemData,
        userId
      );
      if (!createdActionItem) {
        return response
          .status(400)
          .json({ message: "Failed to create action item" });
      }
  ...
  async updateActionItem(
    ...
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const updatedActionItem = await actionItemsService.updateActionItem(
        actionItemData,
        userId
      );
  ...
  async deleteActionItem(
    ...
    try {
      ...
      return response
        .status(200)
        .json({ message: "Action item deleted successfully" });
    } catch (error) {
      next(error);
    }
  }
  ...
  async createActionItemsUpdates(
    ...
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const updates = await actionItemsService.createActionItemsUpdates(
        body,
        userId
      );
  ...
  async updateActionItemsUpdates(
    request: CustomRequest,
    response: Response,
    next: NextFunction
  ) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const updatedUpdates = await actionItemsService.updateActionItemsUpdates(
        body,
        userId
      );
  ...
  async deleteActionItemsUpdates(
    request: CustomRequest,
    response: Response,
    next: NextFunction
  ) {
    try {
      ...
      return response
        .status(200)
        .json({ message: "Action item updates deleted successfully" });
    } catch (error) {
      next(error);
    }
  }

  async migrateActionItemsToIssues(
    request: CustomRequest,
    response: Response,
    next: NextFunction
  ) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const migratedIssue = await actionItemsService.migrateActionItemsToIssues(
        request.body,
        userId
      );
  ...
  async bulkUpdateActionItems(
    ...
    try {
      ...
      if (!ids || !Array.isArray(ids) || ids.length === 0) {
        return response
          .status(400)
          .json({ message: "IDs array is required and must not be empty" });
      }

      if (!updateData || Object.keys(updateData).length === 0) {
        return response
          .status(400)
          .json({ message: "Update data is required and must not be empty" });
      }
  ...
  async updateAssignee(request: any, response: Response, next: NextFunction) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const result = await actionItemsService.updateAssignee(
        id,
        assignee,
        userId
      );
  ...
  async updateUrgency(request: any, response: Response, next: NextFunction) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const result = await actionItemsService.updateUrgency(
        id,
        urgency,
        userId
      );
  ...
  async updateMistake(request: any, response: Response, next: NextFunction) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const result = await actionItemsService.updateMistake(
        id,
        mistake,
        userId
      );
  ...
  // У ActionItemsController
  async exportActionItemsToExcel(
    request: CustomRequest,
    response: Response,
    next: NextFunction
  ) {
    try {
      ...
      const actionItemsService = new ActionItemsService();
      const csvBuffer = await actionItemsService.exportActionItemsToExcel(
        filters
      );
      ...
      response.setHeader(
        "Content-Disposition",
        "attachment; filename=action_items_export.csv"
      );
      response.send(csvBuffer);
    } catch (error) {
      response.status(500).json({ error: "Failed to export action items" });
    }
  }
}
