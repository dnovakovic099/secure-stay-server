...
export class AsanaClient {
    ...
    constructor() {
        ...
        if (!pat) {
            logger.warn('ASANA_PAT environment variable is not set. Asana integration will be disabled.');
            this.projectId = '';
            this.sectionId = '';
            return;
        }
    ...
    /**
     * Check if Asana client is properly configured
     */
    ...
    async createTask(payload: {
        ...
        if (!this.isConfigured()) {
            logger.warn('Asana client is not configured. Skipping task creation.');
            return null;
        }
