...
export const validateCreateContact = (request: Request, response: Response, next: NextFunction) => {
    const schema = Joi.object({
        ...
        vendorAddress: Joi.string().optional().allow(null, ""),
...
export const validateGetContacts = (request: Request, response: Response, next: NextFunction) => {
    const schema = Joi.object({
        ...
        website_name: Joi.array().items(Joi.string()).optional(),
        rate: Joi.string().optional(),
        rateOperator: Joi.string().valid('equal', 'between', 'moreThan', 'lessThan').optional(),
        rateFrom: Joi.string().optional(),
        rateTo: Joi.string().optional(),
        ...
        serviceType: Joi.array().items(Joi.string().valid("Full", "Pro", "Launch")).min(1).optional(),
        email: Joi.string().email().optional().allow(null),
        source: Joi.array().items(Joi.string().valid("Owner", "Turno", "LL")).optional().allow(null),
        vendorAddress: Joi.string().optional().allow(null, ""),
        keyword: Joi.string().optional(),
        notesKeyword: Joi.string().optional(),
        createdBy: Joi.array().items(Joi.string()).optional(),
        updatedBy: Joi.array().items(Joi.string()).optional(),
