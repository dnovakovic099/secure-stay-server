...
export class UsersService {
  ...
  async updateUserStatus(request: Request) {
    ...
    try {
      ...
      const data = await this.userInfoRepository.update(userId, {
        status: status == 1 ? 0 : 1,
      });
    ...
    try {
      if (Array.isArray(userIds)) {
        await appDatabase.transaction(
          async (updateUserTransaction: EntityManager) => {
            await Promise.all(
              userIds?.map(async (user) => {
                await updateUserTransaction.update(UsersInfoEntity, user, {
                  status: userStatus,
                });
              })
            );
