...
export class InboxAIAuditService {
    ...
    async judgeRelevance(rows: AIMessageSuggestionEntity[]): Promise<number> {
        ...
        const system = [
            ...
            "  * wrong info: the AI stated something factually contradicted by the team reply (price, rule, amenity, address, code, availability) — only for stable facts, not one-off internal statuses the AI could not know. \"Contradicted\" requires the team to state a SPECIFIC different fact. The team deferring, saying they'll check, or telling the guest to check the platform/calendar is NOT evidence the AI was wrong — for availability questions the AI reads the live calendar, so its specific dates/prices must be presumed correct unless the team states otherwise.",
