...
export class UserSubscriptionService {
    ...
    private userSubscriptionInfoRepo = appDatabase.getRepository(UserSubscriptionInfo);
    private userSubscriptionCheckoutSession = appDatabase.getRepository(UserSubscriptionCheckoutSession);
    ...
    async createUserSubscriptionCheckoutSession(planId: string, userId: string) {
        ...
        const checkoutSession = await this.userSubscriptionCheckoutSession.findOne({ where: { userId } });
    ...
    async saveUserSubscriptionInfo(userId: string) {
        const sessionInfo = await this.userSubscriptionCheckoutSession.findOne({ where: { userId } });
        const session = await this.stripe.checkout.sessions.retrieve(sessionInfo.sessionId);
        ...
        if (session.payment_status === 'paid') {
            ...
            userSubscriptionInfo.startDate = moment.unix(subscription.current_period_start).format('YYYY-MM-DD');
            userSubscriptionInfo.endDate = moment.unix(subscription.current_period_end).format('YYYY-MM-DD');
            const durationInSeconds = subscription.current_period_end - subscription.current_period_start;
            userSubscriptionInfo.durationInDays = moment.duration(durationInSeconds, 'seconds').asDays();
            userSubscriptionInfo.created_at = new Date();
            userSubscriptionInfo.updated_at = new Date();

            await this.userSubscriptionInfoRepo.save(userSubscriptionInfo)
                .then(async () => {
    ...
    async getUserSubscriptionInfo(userId: string): Promise<object> {
        const userSubscription = await this.userSubscriptionInfoRepo.findOne({ where: { userId } });
        if (!userSubscription) {
            return {
                isExpired: true,
                subscription: null,
    ...
    async getUserInvoices(userId: string) {
        ...
        const invoices = await this.stripe.invoices.list({ customer: userSubscription.customerId });
        const upcoming_invoice = await this.stripe.invoices.retrieveUpcoming({ customer: userSubscription.customerId });
        ...
        paidInvoices.forEach((invoice) => {
            result.push({
                paid_at: moment.unix(invoice.status_transitions.paid_at).format('MMMM D, YYYY'),
